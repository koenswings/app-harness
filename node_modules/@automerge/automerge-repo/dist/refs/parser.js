 ...
 */
const SEGMENT_CODECS = [indexCodec, matchCodec, cursorsCodec, keyCodec];
...
export function parsePath(path) {
    ...
    if (!trimmed) {
        throw new Error("Invalid path: path cannot be empty or consist only of slashes");
    }
    if (trimmed.includes("//")) {
...
export function parseHeads(heads) {
    return heads ? heads.split("|") : undefined;
}
// ⇧ Parse --- Serialize ⇩
export function serializeHeads(heads) {
    return heads.length > 0 ? `#${heads.join("|")}` : "";
}
export function parseRefUrl(url) {
    ...
    if (!match) {
        throw new Error(`Invalid ref URL: ${url}\n` +
...
export function stringifyRefUrl(documentId, segments, heads) {
