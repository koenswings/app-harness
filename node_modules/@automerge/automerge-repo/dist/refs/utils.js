 ...
 * ```
 */
export function cursor(start, end) {
 ...
 * ```
 */
export function refFromUrl(handle, url) {
    const { documentId, segments, heads } = parseRefUrl(url);
    if (documentId !== handle.documentId) {
        throw new Error(`URL documentId "${documentId}" does not match handle's documentId "${handle.documentId}"`);
    }
    // If URL has heads, get a time-traveled handle
    const targetHandle = heads ? handle.view(encodeHeads(heads)) : handle;
    return new RefImpl(targetHandle, segments);
}
/**
 ...
 * ```
 */
export function refFromString(docHandle, path) {
    const url = docHandle.url;
    const { segments } = parseRefUrl(`${url}/${path}`);
    return new RefImpl(docHandle, segments);
}
/**
