...
/**
 * Wait until the named instance reaches Running state.
 *
 * Uses docker ps to poll container status — reliable and sufficient for current use.
 *
 * NOTE: Axle's review recommended using the Automerge store client for Running status
 * detection. This is the correct long-term approach (it uses the engine's own state
 * machine rather than just docker process state). However, doing so from outside the
 * engine requires initializing the Automerge WASM module (`Automerge.use()`), which
 * means importing the full engine initialization sequence. This creates a fragile
 * cross-repo dependency that the design doc explicitly prohibits.
 *
 * The correct fix is for the engine to expose a lightweight HTTP endpoint (e.g.
 * GET /api/instance/<id>/status) that returns the Running state without requiring
 * the full Automerge stack. Tracked as a future Axle task.
 */
async function waitForInstanceRunning(instanceName, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  log(`Waiting for instance "${instanceName}" to reach Running state...`);
  while (Date.now() < deadline) {
    try {
