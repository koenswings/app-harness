 ...
 *
 * Uses docker ps polling. The Automerge store-watch approach fails on ARM
 * due to WASM initialization requirements outside a bundler context.
 * (Flagged to Axle: needs Automerge.use() init helper or HTTP status endpoint.)
 */
async function waitForInstanceRunning(instanceName, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  log(`Waiting for instance "${instanceName}" to start...`);
  while (Date.now() < deadline) {
    try {
